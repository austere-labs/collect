...
"""
    ...
    @pytest.mark.asyncio
    async def test_code_review_from_file_integration(self, temp_dir, sample_diff_file, mock_llm_results):
        """Test the complete file-based code review workflow as described in Claude command"""
        with patch('reviewer.code_review.llmrunner', new_callable=AsyncMock) as mock_llmrunner, \
             patch('reviewer.code_review.code_review_models_to_mcp') as mock_models_config:
    ...
    def test_claude_command_workflow_documentation(self):
        ...
        assert hasattr(reviewer, 'review_code'), "Should support file-based review (Option B)"
        assert hasattr(reviewer, 'review_diff_from_git'), "Should support git diff review (Option A)"
    ...
    def test_output_file_naming_convention(self, temp_dir, sample_diff_file, mock_llm_results):
    ...
    def test_response_text_extraction_supports_all_models(self):
        ...
        anthropic_response = {
            "content": [{"text": "Claude review content"}]
        }
        assert reviewer.extract_response_text(anthropic_response) == "Claude review content"
        
        # Test OpenAI GPT format  
        openai_response = {
            "choices": [{"message": {"content": "GPT review content"}}]
        }
        ...
        assert reviewer.extract_response_text(gemini_response) == "Gemini review content"
        
        # Test fallback for XAI Grok or unknown formats
        unknown_response = "Direct string response"
        assert reviewer.extract_response_text(unknown_response) == "Direct string response"